...
func (server *ARPServer)getBulkPortConfig() {
        ...
        count := 100
        for {
                bulkInfo, _ := server.asicdClient.ClientHdl.GetBulkPortConfig(asicdServices.Int(curMark), asicdServices.Int(count))
                if bulkInfo == nil {
                        break
                }
                objCnt := int(bulkInfo.Count)
                ...
                for i := 0; i < objCnt; i++ {
                        ...
                        ent.MacAddr = bulkInfo.PortConfigList[i].MacAddr
