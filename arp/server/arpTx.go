...
func (server *ARPServer) sendArpReq(targetIp string, port int) {
        server.logger.Info(fmt.Sprintln("sendArpReq(): sending arp requeust for targetIp ", targetIp, "to port:", port))
        ...
        if pcapHdl == nil {
                server.logger.Info(fmt.Sprintln("Unable to open pcap handle on:", portEnt.IfName, "error:", err))
                return
        }
        defer pcapHdl.Close()
/*
...
func (server *ARPServer) sendArpProbe(port int) {
        ...
        server.logger.Info(fmt.Sprintln("sendArpReq(): sending arp requeust for localIp ", localIp, "to port:", port))
