...
func (mgr *FSRouteMgr) ApplyPolicy(protocol string, policy string, action string,
	...
	for i := 0; i < len(conditions); i++ {
		temp[j] = ribdInt.ConditionInfo{conditions[i].ConditionType,
			conditions[i].Protocol, conditions[i].IpPrefix,
			conditions[i].MasklengthRange}
