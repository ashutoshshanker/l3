...
func (m RIBDServer) UpdateApplyPolicy(info ApplyPolicyInfo, apply bool, db *policy.PolicyEngineDB) {
	...
	logger.Info(fmt.Sprintln("RIB handler UpdateApplyPolicy source:", source, " policy:", policyName,
		" action:", action, " apply:", apply, "conditions: "))
	for j := 0; j < len(conditions); j++ {
		...
		case "MatchProtocol":
			...
			if !ok {
				...
				policyCondition := ribd.PolicyCondition{
					Name:          conditionName,
					ConditionType: conditions[j].ConditionType,
					Protocol:      conditions[j].Protocol,
				}
				_, err = m.ProcessPolicyConditionConfigCreate(&policyCondition, db)
			}
		case "MatchDstIpPrefix":
		case "MatchSrcIpPrefix":
			logger.Info(fmt.Sprintln("IpPrefix:", conditions[j].IpPrefix, "MasklengthRange:",
				conditions[j].MasklengthRange))
		}
		if err == nil {
